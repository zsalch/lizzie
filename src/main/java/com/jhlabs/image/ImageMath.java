...
public class ImageMath {
   ...
   * @param b the bias parameter. 0.5 means no change, smaller values bias towards 0, larger towards
   *     1.
   ...
   * @param b the gain parameter. 0.5 means no change, smaller values reduce gain, larger values
   *     increase gain.
   * @return the output value
   */
  ...
  public static float spline(float x, int numKnots, int[] xknots, int[] yknots) {
    ...
    for (span = 0; span < numSpans; span++) if (xknots[span + 1] > x) break;
  ...
  public static void resample(
      int[] source, int[] dest, int length, int offset, int stride, float[] out) {
    int i, j;
    float intensity;
    float sizfac;
    float inSegment;
